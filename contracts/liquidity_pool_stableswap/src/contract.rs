            ...
            d = ann
                ...
                .fixed_mul_floor(
                    ...
                    &ann.clone()
                        .sub(&one)
                        .mul(&d)
                        .add(&n_coins_256.add(&one).mul(&d_p)),
                );
...
impl ManagedLiquidityPool for LiquidityPool {
    ...
    //  )
    // * `reward_config` - The address of the reward token.
...
impl LiquidityPoolInterfaceTrait for LiquidityPool {
    ...
    fn swap(
        ...
        PoolEvents::new(&e).trade(user, input_coin, token_out, in_amount, dy, dx_fee, 0);
        PoolEvents::new(&e).update_reserves(reserves);
    ...
    fn swap_strict_receive(
        ...
        PoolEvents::new(&e).trade(user, input_coin, token_out, dx, out_amount, dx_fee, 0);
        PoolEvents::new(&e).update_reserves(reserves);
